...
:::tip
All the components in this package are exported in the package `@edumetz16/firecms_ui`,
but also reexported in `@edumetz16/firecms_cloud`, for convenience.
:::
...
`tailwindcss` and the `@edumetz16/firecms_ui` package.

All the components are exported from the `@edumetz16/firecms_ui` package. These are the same components used internally in **FireCMS**.
