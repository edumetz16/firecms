...
The main package has been renamed from `firecms` to `@edumetz16/firecms_cloud` since version `3.0.0-beta.4`.
You can also remove the `@edumetz16/firecms_cli` package, as it is implicitly installed by `@edumetz16/firecms_cloud`.
...
```
{
  ...
  "dependencies": {
    "@edumetz16/firecms_cloud": "^3.0.0-beta",
    "firebase": "^10.9.0",
    "react": "^18.3.1",
    "react-dom": "^18.3.1"
  },
...
```javascript
import { FireCMS } from "@edumetz16/firecms_cloud";
```
...
export default defineConfig({
    ...
    plugins: [
        ...
        federation({
            ...
            shared: [
                "react", "react-dom", "@edumetz16/firecms_cloud", "@edumetz16/firecms_core", "@edumetz16/firecms_firebase", "@edumetz16/firecms_ui",
            ]
        })
    ],
    build: {
...
- The preset now comes from the `@edumetz16/firecms_ui` package. You can update your `tailwind.config.cjs` file to include the new preset:
