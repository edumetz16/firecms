...
Import the `Card` from `@edumetz16/firecms_ui` and wrap the content you wish to display on the card. You can also make the card clickable by providing an `onClick` handler.
